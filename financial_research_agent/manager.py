...
# Add to your existing manager.py

class VoiceFinancialResearchManager(FinancialResearchManager):
    """Enhanced manager with voice capabilities"""
    
    def __init__(self, voice_callback: Optional[Callable] = None):
        super().__init__()
        self.voice_callback = voice_callback
    
    async def run_with_voice(self, query: str) -> None:
        """Run research with voice updates"""
        if self.voice_callback:
            await self.voice_callback("Starting financial research...")
        
        # Your existing research logic with voice updates
        search_plan = await self._plan_searches(query)
        
        if self.voice_callback:
            await self.voice_callback(f"Planning {len(search_plan.searches)} searches...")
        
        search_results = await self._perform_searches(search_plan)
        
        if self.voice_callback:
            await self.voice_callback("Analyzing results and writing report...")
        
        report = await self._write_report(query, search_results)
        verification = await self._verify_report(report)
        
        if self.voice_callback:
            await self.voice_callback("Research complete! Here are the key findings...")
